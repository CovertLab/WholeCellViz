<h1>Contents</h1>
<ul>
<li><a href="#intro">Introduction</a>
	<ul>
	<li><a href="#features">Features</a></li>
	<li><a href="#limitations">Limitations</a></li>
	<li><a href="#starting">Getting started</a></li>	
	<li><a href="#browsers">Supported browsers</a></li>	
	<li><a href="#structuredViz">Structured vizs</a></li>
	</ul>
	</li>
<li><a href="#basic">Basic usage</a>
	<ul>
	<li><a href="#install">Installing &amp; running</a></li>
	<li><a href="#exViz">Example views</a></li>
	<li><a href="#config">Configuring panels</a></li>
	<li><a href="#undoRedo">Undo/redo config</a></li>
	<li><a href="#fullscreen">Expanding panels</a></li>
	<li><a href="#panelInfo">Panel details</a></li>
	<li><a href="#labels">View/hide labels</a></li>
	<li><a href="#userInteraction">Tooltips, mouse clicks</a></li>
	<li><a href="#timeline">Timeline controls</a></li>
    <li><a href="#importingData">Importing data</a></li>
	<li><a href="#exportingGrapics">Exporting graphics</a></li>
	<li><a href="#exportingData">Exporting data</a></li>
	</ul>
	</li>
<li><a href="#advanced">Advanced usage</a>
	<ul>
	<li><a href="#externalData">Loading external data</a></li>
	<li><a href="#newViz">Adding visualizations</a></li>
	<li><a href="#localInstall">Installing locally</a></li>
	<li><a href="#runSims">Running simulations</a></hli>
	</ul>
	</li>
<li><a href="#downloading">Downloading software</a></li>
<li><a href="#citing">Citing WholeCellViz</a></li>
<li><a href="#team">Development team</a></li>
<li><a href="#implementation">Implementation</a></li>
<li><a href="#questions">Questions/comments</a></li>
</ul>